...
class RegisterTenantController extends Controller
{
    ...
    private function uniqueTenantSlug(string $base): string
    {
        ...
        // Use PostgreSQL regex operator ~ instead of MySQL REGEXP
        $taken = Tenant::where('slug', $slug)
            ->orWhereRaw("slug ~ ?", ["^{$slug}-[0-9]+$"])
            ->pluck('slug')
            ->flip();
        ...
        for ($i = 1; isset($taken["{$slug}-{$i}"]); $i++);

        return "{$slug}-{$i}";
    }

    // Oude slug creatie met regex
    // private function uniqueTenantSlug(string $base): string
    // {
    //     $slug = Str::lower($base);
